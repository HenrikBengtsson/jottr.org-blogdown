...
2021 that involves overall improvements and essential preparations for
going forward with some exciting new features that I'm keen to work on
during 2022.
...
spent on checking for available results from these future backends
when the results are not yet available.  If possible, that is, after
making sure it's safe to do so, we might decrease the default timeout
to zero in a later release.
...
R expression templates which then can quickly re-compiled into the
...
moving away from odds-and-ends features, especially the ones that are
holding us back from adding new features. The goal is to ensure that
more code using futures can truly run anywhere, not just on a
...
other things.  I hope to be able to make progress on several of them
during 2022.  If you're curious on what's coming up, see
