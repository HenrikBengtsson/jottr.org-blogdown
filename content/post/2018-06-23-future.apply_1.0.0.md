...
The differences between the sequential base-R implementation and the **future.apply** implementation are minimal.  The **future.apply** package is attached, the parallel plan of four workers is setup, and the `apply()` function is replaced by `future_apply()`, where we specify `future.seed = TRUE` to use statistical sound and numerically reproducible parallel random number generation (RNG).
