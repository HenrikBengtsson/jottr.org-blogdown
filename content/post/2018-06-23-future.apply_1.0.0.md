...
In the **parallel** package there is an example - in `?clusterApply` - showing how to perform a bootstrap simulation in parallel.  With some small modifications to clarify the steps, it looks like:
...
The script defines a function `run1()` that produces 500 bootstrap samples, and then it call this function `nsets` of times, combines the replicated samples in to `cd4.boot`, and at the end it uses `boot.ci()` to summarize the results.
...
We notice a few things about these two code snippets.  First of all, in the parallel code, there are two `library(boot)` calls, one in the main code and one inside the `run1()` function.  The reason for this is to make sure that the **boot** package is also attached in the parallel, background R session when `run1()` is called there.  The **boot** package defines the `boot.ci()` function, as well as the `boot()` function and the `cd4` data.frame - both used inside `run1()`.  If **boot** would not be attached inside the function, we would get an error on `"object 'cd4' not found"` when running the parallel code.  However, in the sequential code, we do not need to worry about this - it just works.  Also, if we later would turn our parallel script into a package, then `R CMD check` would complain if we kept the `library(boot)` call inside the `run1()` function.

Second, the example uses `MASS::mvrnorm()` in `run1()`.  The reason for this is related to the above - if we would have had used `mvrnorm()`, we would have to attach the **MASS** package using `library(MASS)` and we would have had to add it inside `run1()`.  Since there is only one **MASS** function called, it's simply easier and neater to use the form `MASS::mvrnorm()`.
...
Here is the same example parallelized using the **future.apply** package looks like:
...
The differences between the base R, sequential implementation and the **future.apply** implementation is minimal.  The **future.apply** package is attached, the parallel plan of four workers is setup, the `apply()` function is replaced by `future_apply()`, where we specify `future.seed = TRUE` to use statistical sound and numerically reproducible parallel random number generation (RNG).
More importantly, notice how there is no need to worry about which packages need to be attached on the workers and what global variables needs to be exported.  That is all taken care of automatically.
